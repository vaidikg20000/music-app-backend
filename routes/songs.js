...
router.post("/artists/add", [artistsValidation, auth], async (req, res) => {
  try {
...
router.get("/all/top", auth, async (req, res) => {
  try {
    ...
    const getSongs = await pool.query(
      "SELECT * FROM songs_by_user WHERE user_id = $1 LIMIT 10",
      [user_id]
    );
    ...
    for (let i = 0; i < getSongs.rows.length; i++) {
      ...
      let image = await pool.query(
        "SELECT image,name FROM songs WHERE song_id = $1",
        [song_id]
      );
...
router.get("/artists/all/top", auth, async (req, res) => {
  try {
    const artistsInfo = [];
...
router.post("/add", [auth, songValidation], async (req, res) => {
  try {
...
router.delete("/delete/:id", auth, async (req, res) => {
  try {
    const { id } = req.params;
    const response = await pool.query("DELETE FROM songs WHERE song_id = $1", [
      id,
    ]);
